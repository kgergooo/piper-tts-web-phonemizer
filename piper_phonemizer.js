var createPiperPhonemize = (() => {
  ...
  return function (moduleArg = {}) {
    ...
    const phonemizerManifest = moduleArg.phonemizerManifest;

    // Separated language changes
    const requestedLanguage = (moduleArg.language || moduleArg.phonemizerLanguage || moduleArg.espeakLanguage || moduleArg.lang || '')
      .trim()
      .toLowerCase();

    const resolvedLanguage = requestedLanguage || 'en';

    if (!phonemizerManifest || typeof phonemizerManifest !== 'object') {
      throw new Error('createPiperPhonemize: phonemizerManifest must be provided via module options');
    }

    Module.__requestedPhonemizerLanguage = requestedLanguage;
    Module.__resolvedPhonemizerLanguage = resolvedLanguage;
    Module.__phonemizerManifest = phonemizerManifest;
    // Separated language changes
    ...
    (function () {
      ...
      // TODO
      const manifest = Module.__phonemizerManifest;
      if (!manifest) throw new Error(`No phonemizer manifest for ${Module.__resolvedPhonemizerLanguage}`);

      const PACKAGE_NAME_FOR_LANG = `piper_phonemize-${Module.__resolvedPhonemizerLanguage}.data`;
      // TODO
      ...
      var loadPackage = function (metadata) {
        ...
        var PACKAGE_NAME = PACKAGE_NAME_FOR_LANG;
        var REMOTE_PACKAGE_BASE = PACKAGE_NAME_FOR_LANG;
        ...
        function runWithFS() {
          ...
          metadata.files.forEach(({ filename }) => {
            const idx = filename.lastIndexOf('/');
            if (idx > 0) directories.add(filename.slice(0, idx));
          });
          directories.forEach(ensurePath);
